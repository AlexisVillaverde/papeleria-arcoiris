...
class InventarioCRUD {
    ...
    public function obtenerProductos() {
        try {
            ...
            error_log('SQL ejecutado correctamente. Productos encontrados: ' . count($result));
            return $result;
        } catch(PDOException $e) {
    ...
    public function actualizarStock($id, $stock) {
        try {
            $sql = "UPDATE productos SET stock = :stock, fecha_actualizacion = NOW() WHERE id_producto = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id);
            $stmt->bindParam(':stock', $stock);
            return $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch($action) {
        ...
        case 'actualizar_stock':
            $resultado = $inventario->actualizarStock($_POST['id'], $_POST['stock']);
            echo json_encode(['success' => $resultado]);
            break;
